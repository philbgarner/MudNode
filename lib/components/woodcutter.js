...
class Woodcutter extends Component {
    constructor (params) {
        ...
        console.log('created woodcutter', this.parent, this.location)
    ...
    Cut (entity, timeout) {
        setTimeout(() => {
            ...
            let parent = entities.getMobileByUuid(this.parent)
            room.SendMessage(`A ${parent.name} cuts the ${entity.name} some more.`)
            if (this.progress > 1) {
                this.progress = 0
                this.occupied = false
                room.SendMessage(`A ${parent.name} cuts the ${entity.name} all the way down.`)
                let e = 0
    ...
    Update () {
        ...
        if (room !== null) {
            let parent = entities.getMobileByUuid(this.parent)
            console.log('parent', parent, this.parent)
