...
import fs from 'fs'
import { entities, config, users, grammar, rooms, areas, templates, MobileTemplate } from './mudnode.js'
...
function save(params) {
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            if (params.savemobiletemplates || params.all) {
                let mobiles = templates.getMobileTemplates()
                let mobs = {}
                for (let m in mobiles) {
                    mobs[m] = new MobileTemplate(mobiles[m]).toJSON()
                }
                fs.writeFileSync(path.join(dir, configData.saveMobileTemplates), JSON.stringify(mobs, null, configData.indentamt))
            }
            ...
            if (params.saveroomtemplates || params.all) {
                fs.writeFileSync(path.join(dir, configData.saveRoomTemplates), JSON.stringify(templates.getRoomTemplates(), null, configData.indentamt))
            }
...
function load() {
    return new Promise((resolve, reject) => {
        try {
            ...
            if (fs.existsSync(path.join(dir, configData.saveMobileTemplates))) {
                let templateBuf = fs.readFileSync(path.join(dir, configData.saveMobileTemplates))
                templates.loadMobileTemplates(templateBuf)
            }
