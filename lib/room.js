import { rooms, areas, entities, Entity, grammar, components } from './mudnode.js'
import { v4 } from 'uuid'
...
class Room extends Entity {
    constructor (params) {
        ...
        this.colour = params.colour ? params.colour : '#0991c1'
    ...
    Describe(ws) {
        ws.send(`${grammar.process(this.name, { room: this.uuid }).sentence}\n${grammar.process(this.description, { room: this.uuid }).sentence}`)
        ...
        for (let e in this.exits) {
            let rm = rooms.getRoom(this.exits[e])
            if (rm) {
                exits += `${exits.length === 0 ? 'T' : 't'}o the <span style='color: ${rm.colour}'>${e.toLowerCase()}</span> ${exits.length === 0 ? 'you see ': ''}${rm.name.toLowerCase()}, `
            }
        }
        ...
        ws.send(`${exits.substring(0, exits.length - 2)}.`)
    ...
    static fromJSON(json) {
        ...
        for (let c in room.components) {
            try {
                let comp = components.CreatefromJSON(room.components[c])
                if (comp) {    
                    cmp.push(comp)
                } else {
