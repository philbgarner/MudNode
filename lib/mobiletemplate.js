...
class MobileTemplate {
    ...
    GenerateMobile(room) {
        ...
        for (let c in this.components) {
            let component = components.Create(this.components[c].type, this.components[c].enabled, mob.uuid)
            for (let p in this.components[c].props) {
                if (typeof this.components[c].props[p] === 'string' && this.components[c].props[p].includes('[')) {
                    component.props[p] = grammar.process(this.components[c].props[p], { 'room': room.uuid, 'mobile': mob.uuid }).sentence
                } else {
